...
/* ================================================================
   Liquid Glass — metaball refraction (React Bits-style demo,
   adapted from a full-window demo into a container-scoped, disposable
   effect). The demo's blue gradient + "Liquid Glass" typography are
   replaced with a dark charcoal that matches the cover background.
   Returns a destroy() that tears everything down.
   ================================================================ */
export function createLiquidGlass(container) {
  ...
  /* ── Background texture: dark charcoal matching the cover ── */
...
void main(){
  ...
  for(int i = 0; i < MAX_N; i++){
    ...
    float contrib = r * r / dSq; field += contrib;
  ...
  const MERGE_RATIO = 0.62, SPLIT_SPEED = 0.013, SPLIT_MIN_R = 0.04, MAX_SPEED = 0.015, BOUNCE = 0.4;
  ...
  /* ── Loop ── */
  let last = performance.now(), acc = 0, paused = false, rafId = 0;
