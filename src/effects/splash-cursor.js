/* ============================================================
   SplashCursor — vanilla port of the React Bits <SplashCursor />.

   A full Navier–Stokes fluid solver in raw WebGL (curl → vorticity →
   divergence → Jacobi pressure → gradient subtract → advection). No
   dependencies, so this is the component's own code with the React shell
   removed: useRef becomes a local, useEffect becomes mount()/destroy(),
   and the JSX wrapper is created imperatively.

   Usage: SplashCursor.mount({ ...options })
   ============================================================ */
(function (global) {
  'use strict';
  ...
  function mount(options) {
    ...
    wrapEl.className = 'splash-cursor';
    wrapEl.style.zIndex = o.zIndex;
    ...
    if (!ctx || !ctx.ext.formatRGBA) { wrapEl.remove(); return null; }
    ...
    function addKeywords(source, keywords) {
      ...
      var str = '';
      keywords.forEach(function (k) { str += '#define ' + k + '\n'; });
      return str + source;
    }
    ...
    function handleTouchStart(e) {
      var touches = e.targetTouches, pointer = pointers[0];
    ...
    function handleTouchEnd(e) {
      var touches = e.changedTouches, pointer = pointers[0];
    ...
    global.addEventListener('mousedown', handleMouseDown);
    global.addEventListener('mousemove', handleMouseMove);
    global.addEventListener('touchstart', handleTouchStart);
    global.addEventListener('touchmove', handleTouchMove, false);
    global.addEventListener('touchend', handleTouchEnd);
    ...
    return {
      destroy: function () {
        isActive = false;
        if (animationFrameId) { cancelAnimationFrame(animationFrameId); animationFrameId = null; }
        global.removeEventListener('mousedown', handleMouseDown);
        global.removeEventListener('mousemove', handleMouseMove);
        global.removeEventListener('touchstart', handleTouchStart);
        global.removeEventListener('touchmove', handleTouchMove);
        global.removeEventListener('touchend', handleTouchEnd);
  ...
  global.SplashCursor = { mount: mount };
})(window);
